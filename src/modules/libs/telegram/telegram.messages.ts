...
export const MESSAGES = {
	welcome:
		`<b>👋 Добро пожаловать в BeaverStream Bot!</b>\n\n` +
		`Чтобы получать уведомления и улучшить ваш опыт использования платформы, давайте свяжем ваш Telegram аккаунт с BeaverStream.\n\n` +
		`Нажмите на кнопку ниже и перейдите в раздел <b>Уведомления</b>, чтобы завершить настройку.`,
	authSuccess: `🎉 Вы успешно авторизовались и Telegram аккаунт связан с BeaverStream!\n\n`,
	...
	follows: (user: User) =>
		`📺 <a href="https://beaverstream.ru/${user.username}">${user.username}</a>`,
	resetPassword: (token: string, metadata: SessionMetadata) =>
		`<b>🔒 Сброс пароля</b>\n\n` +
		`Вы запросили сброс пароля для вашей учетной записи на платформе <b>BeaverStream</b>.\n\n` +
		`Чтобы создать новый пароль, пожалуйста, перейдите по следующей ссылке:\n\n` +
		`<b><a href="https://beaverstream.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
		...
		`Спасибо за использование <b>BeaverStream</b>! 🚀`,
	deactivate: (token: string, metadata: SessionMetadata) =>
		...
		`Спасибо, что пользуетесь <b>BeaverStream</b>! Мы всегда рады видеть вас на нашей платформе и надеемся, что вы останетесь с нами. 🚀\n\n` +
		`С уважением,\n` +
		`Команда BeaverStream`,
	accountDeleted:
		`<b>⚠️ Ваш аккаунт был полностью удалён.</b>\n\n` +
		`Ваш аккаунт был полностью стерт из базы данных <b>BeaverStream<b>. Все ваши данные и информация были удалены безвозвратно. ❌\n\n` +
		...
		`<b><a href="https://beaverstream.ru/account/create">Зарегистрироваться на BeaverStream</a></b>\n\n` +
	...
	streamStart: (channel: User) =>
		...
		`Смотрите здесь: <a href="https://beaverstream.ru/${channel.username}">Перейти к трансляции</a>`,
	newFollowing: (follower: User, followersCount: number) =>
		`<b>У вас новый подписчик!</b>\n\nЭто пользователь <a href="https://beaverstream.ru/${follower.username}">${follower.displayName}</a>\n\nИтоговое количество подписчиков на вашем канале: ${followersCount}`,
	newSponsorship: (plan: SponsorshipPlan, sponsor: User) =>
		`<b>🎉 Новый спонсор!</b>\n\n` +
		`Вы получили новое спонсорство на план <b>${plan.title}</b>.\n` +
		`💰 Сумма: <b>${plan.price} ₽</b>\n` +
		`👤 Спонсор: <a href="https://beaverstream.ru/${sponsor.username}">${sponsor.displayName}</a>\n` +
		`📅 Дата оформления: <b>${new Date().toLocaleDateString()} в ${new Date().toLocaleTimeString()}</b>`,
	enableTwoFactor:
		`🔐 Обеспечьте свою безопасность!\n\n` +
		`Включите двухфакторную аутентификацию в <a href="https://beaverstream.ru/dashboard/settings">настройках аккаунта</a>.`,
	verifyChannel:
		...
		`Спасибо, что вы с нами и продолжаете развивать свой канал вместе с BeaverStream!`
}
